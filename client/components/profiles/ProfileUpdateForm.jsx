...
export default function ProfileUpdateForm ({ profile, currTasks, allTasks, onSubmit }) {
  const { register, handleSubmit, formState: { errors }, unregister } = useForm({
    defaultValues: profile
  })
  const [tasksList, setTasksList] = useState(formatTaskList(profile, currTasks))
  ...
  function formatTaskList (profile, tasks) {
    let output = []

    // sort by task position
    let profileTasksList = profile.tasks.sort((a, b) => a.position - b.position)

    for (let i = 0; i < profileTasksList.length; i++) {
      output.push({ task_id: profileTasksList[i].task_id })
    }
  ...
  const addTask = () => {
    setTasksList([...tasksList, { task_id: allTasks[0].id }])
  }
  ...
  return (
    <>
      <form action='' method='post' onSubmit={handleSubmit(onSubmit)} className='space-y-4 mt-2'>
        ...
        <button className={styles.button} id='create-button'>Update</button>
      </form>
    </>
  )
}
