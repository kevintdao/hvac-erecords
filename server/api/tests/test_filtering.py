...
class TestFilteringAPI(TestCase):
    ...
    def test_api_filter_technician_as_company(self):
        ...
        response = self.client.get(
            reverse("technicians-detail", kwargs={"pk": technician_related.user_id}),
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        technician_not_related = Technician.objects.exclude(
            company=self.company
        ).first()
        response = self.client.get(
            reverse(
                "technicians-detail", kwargs={"pk": technician_not_related.user_id}
            ),
            format="json",
        )
    ...
    def test_api_filter_building_manager_as_company(self):
        ...
        response = self.client.get(
            reverse("managers-detail", kwargs={"pk": manager_related.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(
            reverse("managers-detail", kwargs={"pk": manager_not_related.id}),
            format="json",
        )
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_filter_building_as_company(self):
        ...
        response = self.client.get(
            reverse("buildings-detail", kwargs={"pk": building_related.id}),
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(
            reverse("buildings-detail", kwargs={"pk": building_not_related.id}),
            format="json",
        )
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_filter_unit_as_company(self):
        ...
        response = self.client.get(
            reverse("units-detail", kwargs={"pk": unit_related.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        unit_not_related = Unit.objects.exclude(building__in=buildings).first()
        response = self.client.get(
            reverse("units-detail", kwargs={"pk": unit_not_related.id}), format="json"
        )
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_filter_service_visits_as_company(self):
        ...
        response = self.client.get(
            reverse("visits-detail", kwargs={"pk": visit_related.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        visit_not_related = ServiceVisit.objects.exclude(
            technician__in=technicians
        ).first()
        response = self.client.get(
            reverse("visits-detail", kwargs={"pk": visit_not_related.id}), format="json"
        )
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_filter_tasks_as_company(self):
        ...
        response = self.client.get(
            reverse("tasks-detail", kwargs={"pk": task_related.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        task_not_related = Task.objects.exclude(company=self.company).first()
        response = self.client.get(
            reverse("tasks-detail", kwargs={"pk": task_not_related.id}), format="json"
        )
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_filter_profiles_as_company(self):
        ...
        profiles = Profile.objects.filter(
            company=self.company
        ) | Profile.objects.filter(company=None)
        ...
        response = self.client.get(
            reverse("profiles-detail", kwargs={"pk": profile_related.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        profile_not_related = (
            Profile.objects.exclude(company=self.company).exclude(company=None).first()
        )
        response = self.client.get(
            reverse("profiles-detail", kwargs={"pk": profile_not_related.id}),
            format="json",
        )
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_filter_task_completions_as_company(self):
        ...
        response = self.client.get(
            reverse("completions-detail", kwargs={"pk": completion_related.id}),
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(
            reverse("completions-detail", kwargs={"pk": completion_not_related.id}),
            format="json",
        )
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_filter_profile_plans_as_company(self):
        ...
        response = self.client.get(
            reverse("plans-detail", kwargs={"pk": plan_related.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        plan_not_related = ProfilePlan.objects.exclude(profile__in=profiles).first()
        response = self.client.get(
            reverse("plans-detail", kwargs={"pk": plan_not_related.id}), format="json"
        )
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_filter_profiles_as_manager(self):
        ...
        profiles = Profile.objects.filter(plans__in=plans) | Profile.objects.filter(
            company=None
        )
