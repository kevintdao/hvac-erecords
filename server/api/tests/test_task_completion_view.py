...
class TestTaskCompletionAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.data = {
            "task": 1,
            "service_visit": 1,
            "completed_at": self.service_visit.start_time
            + datetime.timedelta(minutes=1),
            "response": "This is the technician response",
        }
        self.response = self.client.post(
            reverse("completions-list"), self.data, format="json"
        )
    ...
    def test_api_create_task_completion(self):
        ...
        self.assertEqual(
            TaskCompletion.objects.last().response, "This is the technician response"
        )
    ...
    def test_api_create_task_completion_failure(self):
        ...
        self.response = self.client.post(
            reverse("completions-list"), data, format="json"
        )
    ...
    def test_api_get_task_completion(self):
        ...
        response = self.client.get(
            reverse("completions-detail", kwargs={"pk": task_completion.id}),
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_update_task_completion(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            TaskCompletion.objects.last().response, "This is a new technician response"
        )
    ...
    def test_api_update_task_completion_failure(self):
        ...
        new_data = {
            "task": 1,
            "service_visit": 1,
            "completed_at": self.service_visit.end_time
            + datetime.timedelta(minutes=20),
            "response": "This is the technician response",
        }
        response = self.client.put(
    ...
    def test_api_task_completion_not_found(self):
        response = self.client.get(
            reverse("completions-detail", kwargs={"pk": 0}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_api_task_completion_noperm(self):
