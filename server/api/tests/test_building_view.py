...
class TestBuildingAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.response = self.client.post(
            reverse("buildings-list"), self.data, format="json"
        )
    ...
    def test_api_get_building(self):
        ...
        response = self.client.get(
            reverse("buildings-detail", kwargs={"pk": building.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_building_not_found(self):
        response = self.client.get(
            reverse("buildings-detail", kwargs={"pk": 0}), format="json"
        )
    ...
    def test_api_create_building_failure(self):
        ...
        self.response = self.client.post(reverse("buildings-list"), data, format="json")
