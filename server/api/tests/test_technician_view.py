...
class TestTechnicianAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.response = self.client.post(
            reverse("technicians-list"), self.data, format="json"
        )
    ...
    def test_api_create_technician_failure(self):
        ...
        self.response = self.client.post(
            reverse("technicians-list"), data, format="json"
        )
    ...
    def test_api_get_technician(self):
        ...
        response = self.client.get(
            reverse("technicians-detail", kwargs={"pk": technician.user_id}),
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_technician_not_found(self):
        response = self.client.get(
            reverse("technicians-detail", kwargs={"pk": 0}), format="json"
        )
