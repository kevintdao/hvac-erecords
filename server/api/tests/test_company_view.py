...
class TestMaintenanceCompanyAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.data = {
            "users": [
                {"email": "company123@email.com", "password": "password123"},
            ],
        ...
        self.response = self.client.post(
            reverse("companies-list"), self.data, format="json"
        )
    ...
    def test_api_get_company(self):
        ...
        response = self.client.get(
            reverse("companies-detail", kwargs={"pk": company.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_update_company(self):
        ...
        response = self.client.put(
            reverse("companies-detail", kwargs={"pk": company.id}),
            data=new_data,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_company_not_found(self):
        response = self.client.get(
            reverse("companies-detail", kwargs={"pk": 0}), format="json"
        )
    ...
    def test_api_create_company_failure(self):
        ...
        self.response = self.client.post(reverse("companies-list"), data, format="json")
