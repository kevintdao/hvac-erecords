...
class TestProfileAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.data = {
            ...
            "tasks": [{"task_id": 1, "position": 1}, {"task_id": 2, "position": 2}],
        }

        self.response = self.client.post(
            reverse("profiles-list"), self.data, format="json"
        )
    ...
    def test_api_create_profile_failure(self):
        ...
        self.response = self.client.post(reverse("profiles-list"), data, format="json")
        self.assertEqual(self.response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_api_get_profile(self):
        profile = Profile.objects.last()
        response = self.client.get(
            reverse("profiles-detail", kwargs={"pk": profile.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_update_profile(self):
        ...
        new_data = {
            ...
            "tasks": [
                ...
                {"task_id": 1, "position": 2},
                {"task_id": 2, "position": 3},
            ],
        }
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            Profile.objects.last().description,
            "this is the profile for routine for packed units",
        )
    ...
    def test_api_update_profile_failure(self):
        ...
        new_data = {
            "title": "Routine maintenance for packed units",
            "tasks": [{"task_id": 1, "position": 2}, {"task_id": 2, "position": 3}],
        }
        response = self.client.put(
            reverse("profiles-detail", kwargs={"pk": profile.id}),
            data=new_data,
    ...
    def test_api_profile_not_found(self):
        response = self.client.get(
            reverse("profiles-detail", kwargs={"pk": 0}), format="json"
        )
