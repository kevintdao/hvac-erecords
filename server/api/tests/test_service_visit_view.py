...
class TestServiceVisitAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.response = self.client.post(
            reverse("visits-list"), self.data, format="json"
        )
    ...
    def test_api_create_service_visit(self):
        ...
        self.assertEqual(
            ServiceVisit.objects.last().technician, Technician.objects.get(pk=3)
        )
        self.assertEqual(
            ServiceVisit.objects.last().plan, ProfilePlan.objects.get(pk=1)
        )
    ...
    def test_api_create_service_visit_failure(self):
        ...
        self.response = self.client.post(reverse("visits-list"), data, format="json")
    ...
    def test_api_get_service_visit(self):
        ...
        response = self.client.get(
            reverse("visits-detail", kwargs={"pk": visit.id}), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_api_update_service_visit(self):
        ...
        response = self.client.put(
            reverse("visits-detail", kwargs={"pk": visit.id}),
            data=new_data,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            str(ServiceVisit.objects.last().end_time), "2022-03-23 17:41:28+00:00"
        )
        self.assertEqual(ServiceVisit.objects.count(), self.initial_count + 1)
    ...
    def test_api_service_visit_not_found(self):
        response = self.client.get(
            reverse("visits-detail", kwargs={"pk": 0}), format="json"
        )
