...
class ProfileTaskPositionSerializer(serializers.ModelSerializer):
    ...
    task_id = serializers.PrimaryKeyRelatedField(
        write_only=True, source="task", queryset=Task.objects.all()
    )
...
class TaskCompletionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (data["completed_at"] < service_visit.start_time) or (
            service_visit.end_time is not None
            and data["completed_at"] > service_visit.end_time
        ):
            raise serializers.ValidationError(
                "Task completion is not in service visit start/end time range"
            )
        return data
